...
namespace ports
{
	...
	public:
		 ...
		 * with an folder for sub-variables
		 *
		 * @param oSubroutine subroutine which hold the information for variable with SubVar
		 * @param sSubVar defined SubVar from script
		 */
		SubroutineSubVarHolder(SHAREDPTR::shared_ptr<IMeasurePattern> oFolder, const string& sSubVar);
		/**
		 * constructor to define SubVarHolder object
		 * with an subroutine for sub-variables
		 *
		 * @param oFolder folder which hold the information for variable with SubVar
		 * @param sSubVar defined SubVar from script
		 */
		SubroutineSubVarHolder(SHAREDPTR::shared_ptr<IListObjectPattern> oSubroutine, const string& sSubVar);
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->lockObject(file, line, who) : false; };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->unlockObject(locked, file, line); };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->needServer() : false; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->hasServer() : false; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->startingBy(tm, from) : false; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->hasSubVar(subvar) : m_oFolder->hasSubVar(subvar); };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->getSubVar(who, subvar) : m_oFolder->getSubVar(who, subvar); };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->getActCount() : 0; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->needObserver() : false; };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->setObserver(observer); };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->informObserver(observer, folder, subroutine, parameter); };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->removeObserver(observer, folder, subroutine, parameter); };
		/**
		 * return string of defined observer links
		 * for current subroutine
		 */
		OVERWRITE string getObserversString() const
		{ return m_oSubroutine ? m_oSubroutine->getObserversString() : ""; };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->setDeviceAccess(access); };
		/**
		 * ask subroutine whether they has an correct access to device
		 *
		 * @return whether the access is correct
		 */
		virtual bool hasDeviceAccess() const
		{ return m_oSubroutine ? m_oSubroutine->hasDeviceAccess() : false; };
		/**
		 * returning all groups, separately with colons
		 *
		 * @return string of groups
		 */
		virtual string getPermissionGroups()
		{ return m_oSubroutine ? m_oSubroutine->getPermissionGroups() : ""; };
		/**
		 * set ending of configuration when folder thread
		 * of measuring starting
		 */
		virtual void endOfConfigure()
		{ if(m_oSubroutine) m_oSubroutine->endOfConfigure(); };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->setDebug(bDebug); };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->isDebug() : m_oFolder->isDebug(); };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->getSubroutineType() : "FOLDER"; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->getFolderName() : m_oFolder->getFolderName(); };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->getSubroutineName() : ""; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->onlySwitch() : true; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->measure(actValue) : auto_ptr<IValueHolderPattern>(); };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->setValue(value, from); };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->noChange(); };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->setRunningThread(thread); };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->getRunningThread() : m_oFolder.get(); };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->getType() : "FOLDER"; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->needChangingTime() : false; };
		/**
		...
		{ return m_oSubroutine ? m_oSubroutine->range(bfloat, min, max) : false; };
		/**
		...
		{ if(m_oSubroutine) m_oSubroutine->stop(bWait); };
		/**
	...
	private:
		/**
		 * real list object (folder)
		 */
		SHAREDPTR::shared_ptr<IMeasurePattern> m_oFolder;
		/**
		 * real subroutine object
		 */
		SHAREDPTR::shared_ptr<IListObjectPattern> m_oSubroutine;
		/**
		 * definition of sub-variable
		 */
		string m_sSubVar;
		/**
		 * null definition for out string
		 * when anyone ask an folder
		 */
		ostringstream m_sNullOut;
	};
}
